  ...
  try {
    data = await completeJson(
      [
        ...
        "- topic: a concise 3-7 word content idea, like 'Turn rejection into information' or 'Measure progress, not busyness'. Never a truncated source sentence.",
        ...
        "- If one item is only a broad summary of the whole source, set topic to 'Key Takeaways', or omit it when you already have 4 distinct ideas.",
      ].join("\n"),
      `Source content:\n\n${sourceText}`,
      ANALYZE_OPPORTUNITIES_SCHEMA,
    );
  ...
  const data = await completeJson(
    [
      ...
      "- Ground every field in the source. Do not invent claims.",
        "- topic: concise 3-7 word content idea, not a truncated source sentence.",
        ...
        "- onScreenText: 1-3 punchy TikTok/Reels captions (2-6 words) that express the core idea. Do not copy a full source sentence.",
